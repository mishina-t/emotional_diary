...
def home(request: Request):
    ...
    cur.execute("SELECT date, sentiment_score, sentiment_magnitude FROM entries WHERE user_id = ? ORDER BY date DESC LIMIT 30", (user_id,))
    rows = cur.fetchall()
    con.close()
    
    rows = rows[::-1]
    dates = [r[0] for r in rows]
    scores = [r[1] if r[1] is not None else 0 for r in rows]
    moods = [max(r[2] * 200, 5) if r[2] is not None else 5 for r in rows]
...
@app.post("/delete")
def delete_entry(request: Request, date: str = Form(...)):
    user = get_current_user(request)
    if not user:
        raise HTTPException(status_code=401, detail="Not authenticated")
    con = sqlite3.connect(DB_PATH)
